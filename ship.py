...
class Ship(Sprite):
	...
	def __init__(self, x=0, y=0, ship_lives=settings.ship_lives, ship_level=settings.ship_starting_level):
		super().__init__(image.ship[ship_level],x,y,constraints = pygame.Rect(settings.ship_constraints), boundary_behaviour="clamp")
	...
	def reset_firepoints(self):
		...
		if self.level == 1:
			...
			self.fire_points = [(9/106*self.w,54/146*self.h),(self.w/2,0),(95/106*self.w,54/146*self.h)]
			self.bullet_sizes = [1,2,1]
		elif self.level == 3:
			self.fire_points = [(12/133*self.w,71/178*self.h),(23/133*self.w,49/178*self.h),(self.w/2,0),(109/133*self.w,49/178*self.h),(120/133*self.w,71/178*self.h)]
	...
	def shoot_bullets(self):
		...
		for i in range(len(self.fire_points)):
			self.bullets.add(Bullet(self.x+self.fire_points[i][0]-image.bullet[self.bullet_sizes[i]].w/2, self.y+self.fire_points[i][1], speed, self.bullet_sizes[i]))
